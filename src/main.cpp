...
void handleTvocSetRequest(AsyncWebServerRequest *request)
{
  ...
  // Send an empty response with 204 No Content status code
  request->send(204);
}

void handleEco2SetRequest(AsyncWebServerRequest *request)
{
  // Get the submitted eco2 set value
  String submittedEco2Set = request->arg("eco2_set");
  Serial.println(submittedEco2Set);

  // Check if the submitted eco2 set value is not empty
  if (submittedEco2Set.length() > 0)
  {
    // Update the eco2 set value
    eCO2_SET = submittedEco2Set.toInt();
...
void setup()
{
  ...
  server.on("/set-eco2", HTTP_POST, handleEco2SetRequest);
